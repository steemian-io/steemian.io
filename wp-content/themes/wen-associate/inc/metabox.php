...
if ( ! function_exists( 'wen_associate_save_theme_settings_meta' ) ) :
   ...
   * @since  WEN Associate 1.0
   */
  function wen_associate_save_theme_settings_meta( $post_id, $post ) {

  	// Verify nonce.
  	if (
  		! ( isset( $_POST['wen_associate_theme_settings_meta_box_nonce'] )
  		&& wp_verify_nonce( sanitize_key( $_POST['wen_associate_theme_settings_meta_box_nonce'] ), basename( __FILE__ ) ) )
  	) {
  		return;
  	}
...
add_action( 'save_post', 'wen_associate_save_theme_settings_meta', 10, 2 );
